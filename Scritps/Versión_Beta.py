...
EXCEL_PATH  = r"C:/Users/adelarosa/Documents/Reportes/Dashboards/DashboardVentasDiarias_97/09_Septiembre/15-09-2026/Dataset_97.xlsx"
OUTPUT_PATH = r"C:/Users/adelarosa/Documents/Reportes/Dashboards/DashboardVentasDiarias_97/09_Septiembre/15-09-2026/index.html"
BOL_EXCLUIR = ["BOLEUCH", "BOLEUGDE", "BOLEUMIN"]
FECHA_BASE  = date(2026, 8, 31)
ES_CIERRE_MES = True
CLAVE_SUC_SIN_TICKETS = [300]
LOGO_PATH = r"C:/Users/adelarosa/Documents/Reportes/Dashboards/DashboardVentasDiarias_97/Logos/logo.png"
...
body{font-family:'Segoe UI',system-ui,sans-serif;background:#FEFBFC;color:#262123;min-height:100vh}
.sticky-header{position:sticky;top:0;z-index:1000;isolation:isolate}
header{background:#E31B23;padding:1.2rem 2rem;display:grid;grid-template-columns:1fr auto 1fr;column-gap:1rem;align-items:center;box-shadow:0 3px 20px rgba(227,27,35,0.2)}
.header-left{min-width:0}
.header-left h1{font-size:1.15rem;font-weight:700;color:#fff;letter-spacing:.03em}
.header-left p{font-size:.72rem;color:#FAECEE;margin-top:4px;letter-spacing:0.02em}
...
.hdate{font-size:.72rem;color:#FAECEE;font-weight:500}
.header-logo{display:flex;align-items:center;justify-content:center;min-width:0}
.header-logo img{height:5.25rem;width:auto;display:block}
.filter-bar{background:#fff;border-bottom:1px solid #F2EAEC;padding:.75rem 2rem;display:flex;gap:.8rem;flex-wrap:wrap;align-items:center;justify-content:center;box-shadow:0 2px 4px rgba(227,27,35,0.02)}
.filter-bar + .filter-bar{border-top:1px solid #F7EFF0;box-shadow:none}
.suc-label{font-size:.67rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#E31B23;margin-right:.25rem;white-space:nowrap}
.suc-hint{font-size:.62rem;color:#C09CA5;font-weight:500;white-space:nowrap;margin-right:.4rem}
.suc-btns{display:flex;gap:6px;flex-wrap:wrap;justify-content:center}
.suc-btn{display:inline-flex;align-items:center;padding:6px 13px;border-radius:20px;border:1px solid #EBDADC;background:#FCF6F7;color:#756567;font-size:.72rem;font-weight:600;cursor:pointer;transition:all .2s;font-family:inherit;white-space:nowrap}
.suc-btn.active{background:#E31B23 !important;border-color:#E31B23 !important;color:#fff !important;font-weight:700;box-shadow:0 3px 10px rgba(227,27,35,0.2)}
.mes-btn{display:inline-flex;align-items:center;padding:5px 11px;border-radius:20px;border:1px solid #EBDADC;background:#FCF6F7;color:#756567;font-size:.69rem;font-weight:600;cursor:pointer;transition:all .2s;font-family:inherit;white-space:nowrap}
.mes-btn.active{background:#504345 !important;border-color:#504345 !important;color:#fff !important;font-weight:700}
.suc-btn.oculto,.mes-btn.oculto{display:none}
.suc-sep{width:1px;height:20px;background:#EBDDE0;margin:0 .1rem;flex-shrink:0}
.btn-all{padding:6px 13px;border-radius:20px;border:1px solid #E31B23;background:#fff;color:#E31B23;font-size:.7rem;font-weight:700;cursor:pointer;font-family:inherit;transition:all .2s;white-space:nowrap}
.btn-all.dark{border-color:#504345;color:#504345}
.tabs-nav{background:#fff;border-bottom:2px solid #EDE5E6;padding:0 2rem;display:flex;justify-content:center;gap:0;flex-wrap:wrap}
.tab-nav-btn{padding:.85rem 1.6rem;font-size:.82rem;font-weight:700;color:#8C8082;cursor:pointer;border:none;background:none;font-family:inherit;border-bottom:3px solid transparent;margin-bottom:-2px;transition:all .2s;letter-spacing:.02em}
.tab-nav-btn.active{color:#E31B23;border-bottom-color:#E31B23}
.tab-nav-btn:hover:not(.active){color:#504345;border-bottom-color:#EBDADC}
...
.kpi{background:#fff;border-radius:12px;padding:1.1rem 1.2rem;border:1px solid #EDE5E6;position:relative;overflow:hidden;box-shadow:0 2px 6px rgba(0,0,0,0.015);flex:1 1 170px;max-width:215px;min-width:0;text-align:center}
.kpi::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--ac,#E31B23)}
.kpi-label{font-size:.63rem;font-weight:700;text-transform:uppercase;letter-spacing:.09em;color:#8C8082;margin-bottom:6px}
.kpi-value{font-size:1.4rem;font-weight:700;color:#262123;line-height:1}
.kpi-value.purple{color:#E31B23}
.kpi-value.gold{color:#aa7300}
.kpi-sub{font-size:.67rem;color:#8C8082;margin-top:5px}
.kpi-note{font-size:.68rem;color:#8C8082;margin:-.6rem 0 1rem;font-style:italic}
.tc{background:#fff;border-radius:12px;padding:1.2rem 1.6rem;border:1px solid #EDE5E6;margin-bottom:.9rem;box-shadow:0 2px 6px rgba(0,0,0,0.015)}
.card-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:1.1rem;gap:1rem;flex-wrap:wrap}
.card-title{font-size:.88rem;font-weight:700;color:#262123;letter-spacing:0.01em}
.card-sub{font-size:.7rem;color:#8C8082;margin-top:2px}
.note-bol{font-size:.67rem;background:#FCF9F9;color:#E31B23;border:1px solid #EBDDE0;padding:4px 10px;border-radius:6px;white-space:nowrap;font-weight:500}
.metric-tabs{display:flex;gap:4px;background:#F5EFEF;padding:3px;border-radius:8px;border:1px solid #EBE1E2}
.tab-btn{background:none;border:none;padding:5px 12px;font-size:.72rem;font-weight:600;color:#756A6C;cursor:pointer;border-radius:6px;font-family:inherit;transition:all .15s}
.tab-btn.active{background:#fff;color:#E31B23;box-shadow:0 2px 5px rgba(227,27,35,0.1);font-weight:700}
table{width:100%;border-collapse:collapse;font-size:.78rem;min-width:100%}
thead tr{background:#fdfcfd;border-bottom:2px solid #EDE5E6}
thead th{text-align:left;padding:9px 11px;font-size:.63rem;font-weight:700;text-transform:uppercase;letter-spacing:.09em;color:#E31B23;white-space:nowrap}
thead th.r{text-align:right}
tbody tr{border-bottom:1px solid #F8F4F4;transition:background .15s}
tbody tr:last-child{border-bottom:none}
tbody tr:hover{background:#FFFBFB}
tbody tr.total-row{background:linear-gradient(90deg,#FEFBFC,#fff);border-top:2px solid #D1ADB2}
tbody tr.total-row td{font-weight:700;color:#E31B23}
td{padding:9px 11px;color:#544C4E}
td.r{text-align:right;font-variant-numeric:tabular-nums}
td.date{font-weight:600;color:#262123;white-space:nowrap}
td.dayname{font-size:.68rem;color:#8C8082;white-space:nowrap}
td.art-code{font-family:'Consolas',monospace;font-size:.72rem;color:#E31B23;font-weight:700;white-space:nowrap}
td.art-desc{font-size:.75rem;color:#544C4E;line-height:1.4;word-break:break-word;min-width:200px}
td.art-fab{font-size:.72rem;color:#756567;word-break:break-word;min-width:120px}
td.rank{font-size:.78rem;font-weight:800;color:#8C8082;width:32px;text-align:center}
td.rank.top3{color:#E31B23}
...
.cc{background:#fff;border-radius:12px;padding:1.2rem 1.6rem;border:1px solid #EDE5E6;box-shadow:0 2px 6px rgba(0,0,0,0.015)}
...
.lines-table-box,.lines-chart-box{background:#fff;border-radius:12px;padding:1.2rem 1.6rem;border:1px solid #EDE5E6;box-shadow:0 2px 6px rgba(0,0,0,0.015);display:flex;flex-direction:column;min-width:0}
.hist-card{background:#fff;border-radius:12px;padding:1.2rem 1.6rem;border:1px solid #EDE5E6;box-shadow:0 2px 6px rgba(0,0,0,0.015)}
...
.empty{text-align:center;padding:2.5rem;color:#8C8082;font-size:.82rem}
.section-divider{border:none;border-top:2px solid #F3E9EB;margin:2rem 0}
.top-header-card{background:#fff;border-radius:12px;padding:1rem 1.6rem;border:1px solid #EDE5E6;box-shadow:0 2px 6px rgba(0,0,0,0.015);margin-bottom:1rem}
.top-layout{display:block}
.top-table-box{background:#fff;border-radius:12px;padding:1.2rem 1.6rem;border:1px solid #EDE5E6;box-shadow:0 2px 6px rgba(0,0,0,0.015);margin-bottom:1rem}
.top-bubble-box{background:#fff;border-radius:12px;padding:1.2rem 1.6rem;border:1px solid #EDE5E6;box-shadow:0 2px 6px rgba(0,0,0,0.015);display:flex;flex-direction:column;min-height:0}
.badge-rank{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;font-size:.68rem;font-weight:800;background:#F7EFF0;color:#8C8082}
.badge-rank.r1{background:#E31B23;color:#fff}
.badge-rank.r2{background:#B85252;color:#fff}
.badge-rank.r3{background:#D17D84;color:#fff}
.top-kpi-row{display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem;margin-bottom:1rem}
.top-kpi{background:#fff;border-radius:10px;padding:.9rem 1.1rem;border:1px solid #EDE5E6;position:relative;overflow:hidden;box-shadow:0 2px 6px rgba(0,0,0,0.015);text-align:center}
.top-kpi::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--ac,#E31B23)}
.top-kpi .kpi-label{font-size:.6rem}
.top-kpi .kpi-value{font-size:1.1rem}
.legend-gradient{display:inline-flex;align-items:center;gap:6px;font-size:.67rem;color:#756567;white-space:nowrap}
.legend-bar{width:70px;height:8px;border-radius:4px;background:linear-gradient(90deg,#F0D3D6,#7A1A23)}
tr.lc-parent{cursor:pointer}
tr.lc-parent:hover{background:#FFFBFB}
tr.lc-child td{background:#FCFAFA;color:#756567}
tr.lc-child td:nth-child(2){padding-left:26px}
.lc-icon{display:inline-block;width:14px;color:#E31B23;font-weight:700}
...
.fab-rank-num{font-size:.68rem;color:#8C8082;font-weight:700}
.table-scale-wrap{
  width:100%;
  overflow:hidden;
}
...
<div class="filter-bar" id="filter-bar-mes">
  <span class="suc-label" style="color:#504345">Meses · Histórico</span>
...
<div class="main">
  <div class="tc" style="margin-bottom:.9rem">
    ...
    <div class="top-kpi-row" style="grid-template-columns:repeat(4,1fr)">
      <div class="top-kpi" style="--ac:#504345"><div class="kpi-label">Ventas $</div><div class="kpi-value" id="p-ventas">—</div><div class="kpi-sub">Acumulado mes en curso</div></div>
      <div class="top-kpi" style="--ac:#E31B23"><div class="kpi-label">Pronóstico de cierre</div><div class="kpi-value purple" id="p-pronostico">—</div><div class="kpi-sub">Suma de pronósticos</div></div>
...
<div class="main">
  ...
  <div class="kpi-grid">
    <div class="kpi" style="--ac:#AD6268"><div class="kpi-label">Unidades vendidas</div><div class="kpi-value" id="k-uni">—</div><div class="kpi-sub">Ventas mes en curso</div></div>
    <div class="kpi" style="--ac:#E31B23"><div class="kpi-label">Ventas $</div><div class="kpi-value purple" id="k-ventas">—</div><div class="kpi-sub" id="k-ventas-s">—</div></div>
    <div class="kpi" style="--ac:#821E25"><div class="kpi-label">Utilidad</div><div class="kpi-value purple" id="k-util">—</div><div class="kpi-sub" id="k-util-sub">Margen: —</div></div>
    <div class="kpi" style="--ac:#504345"><div class="kpi-label">Tickets</div><div class="kpi-value" id="k-tkt">—</div><div class="kpi-sub" id="k-tkt-sub">Volumen de ventas</div></div>
    <div class="kpi" style="--ac:#A87C80"><div class="kpi-label">Ventas $ promedio por ticket</div><div class="kpi-value" id="k-vtkt">—</div><div class="kpi-sub" id="k-vtkt-sub">Ventas $ ÷ Tickets</div></div>
    <div class="kpi" style="--ac:#D4A3AA"><div class="kpi-label">Unidades promedio por Ticket</div><div class="kpi-value" id="k-utkt">—</div><div class="kpi-sub" id="k-utkt-sub">Unidades ÷ Tickets</div></div>
  </div>
  <div class="tc">
    <div class="card-head">
...
<div class="main">
  ...
  <div class="kpi-grid">
    <div class="kpi" style="--ac:#AD6268"><div class="kpi-label">Unidades vendidas</div><div class="kpi-value" id="ka-uni">—</div><div class="kpi-sub">Acumulado según selección</div></div>
    <div class="kpi" style="--ac:#E31B23"><div class="kpi-label">Ventas $</div><div class="kpi-value purple" id="ka-ventas">—</div><div class="kpi-sub" id="ka-ventas-s">—</div></div>
    <div class="kpi" style="--ac:#821E25"><div class="kpi-label">Utilidad</div><div class="kpi-value purple" id="ka-util">—</div><div class="kpi-sub" id="ka-util-sub">Margen: —</div></div>
    <div class="kpi" style="--ac:#504345"><div class="kpi-label">Tickets</div><div class="kpi-value" id="ka-tkt">—</div><div class="kpi-sub" id="ka-tkt-sub">Volumen de ventas</div></div>
    <div class="kpi" style="--ac:#A87C80"><div class="kpi-label">Ventas $ promedio por ticket</div><div class="kpi-value" id="ka-vtkt">—</div><div class="kpi-sub" id="ka-vtkt-sub">Ventas $ ÷ Tickets</div></div>
    <div class="kpi" style="--ac:#D4A3AA"><div class="kpi-label">Unidades promedio por Ticket</div><div class="kpi-value" id="ka-utkt">—</div><div class="kpi-sub" id="ka-utkt-sub">Unidades ÷ Tickets</div></div>
  </div>
  <div class="tc">
    <div class="card-head">
...
<div class="main">
  <div class="top-kpi-row">
    <div class="top-kpi" style="--ac:#E31B23"><div class="kpi-label">Línea Líder</div><div class="kpi-value purple kv-txt" id="tk2-linea-nombre" style="font-size:1.05rem">—</div><div class="kpi-sub" id="tk2-linea-sub">—</div></div>
    <div class="top-kpi" style="--ac:#852427"><div class="kpi-label">Categoría Líder</div><div class="kpi-value kv-txt" id="tk2-cat-nombre" style="color:#852427;font-size:1.05rem">—</div><div class="kpi-sub" id="tk2-cat-sub">—</div></div>
...
<div class="main">
  <div class="top-kpi-row">
    <div class="top-kpi" style="--ac:#E31B23"><div class="kpi-label">Ventas $ · Fabricantes al 50%</div><div class="kpi-value purple" id="tk3-ventas">—</div><div class="kpi-sub">Suma de fabricantes hasta cubrir el 50%</div></div>
...
<footer style="text-align:center;padding:1.4rem 2rem 2rem;font-size:.7rem;color:#AD9C9F;border-top:1px solid #F2EAEC;margin-top:1rem">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const PALETTE = [
        '#E31B23','#2196A8','#E05C2A','#3A7D44','#C4922A',
        '#1A559E','#B03060','#4E8C6E','#6B4226','#5C5FA8',
        '#8E3A59','#2E7D60','#A04010','#3D6B99','#7A6F1E',
        '#9C3D6B','#1E6B7A','#8E3A4D','#3D7A4A','#8E5A1E',
    ];
    const PALETTE_LINEAS = [
        '#852427','#E31B23','#B84046','#CC5259','#E0666F',
        '#755A5D','#917478','#AD9095','#601A24','#D47F82'
    ];
    ...
    const PREMIUM_TOOLTIP_OPTS = {
        backgroundColor:'#ffffff',titleColor:'#262123',bodyColor:'#544C4E',
        borderColor:'#EDE5E6',borderWidth:1,padding:10,cornerRadius:8,
    ...
    function salesColor(t){
        ...
        const c1 = [238,208,212], c2 = [122,26,35];
    ...
    function updatePresupuesto(){
        ...
        const rows = data.map(r => {
            ...
            return `<tr>
                ...
                <td class="r" data-label="Alcance actual">${tienePresupuesto ? `<span class="pill ${cls}">${fP(cump)}</span>` : '<span style="color:#C9BCBD">—</span>'}</td>
        ...
        const totCumplimientoTd = tB>0 ? `<span class="pill ${totCls}">${fP(totCump)}</span>` : '<span style="color:#C9BCBD">—</span>';
        ...
        charts['objetivos'] = new Chart(document.getElementById('chart-objetivos'), {
            ...
            options: {
                ...
                scales: {
                    x: {
                        grid: { display: false },
                        ticks: { font: { size: 9 }, color: '#8C8082', maxRotation: 60, minRotation: 45, autoSkip: false }
                    },
                    y: {
                        grid: { color: '#F8F4F4' },
                        suggestedMax: maxVal * 1.1,
                        ticks: { font: { size: 9 }, color: '#8C8082', callback: v => fM(v) }
                    }
                }
            }
        });
    ...
    function updateTabla(dates, byDate, sinTicketsNombres){
        ...
        const rows = valid.map(d => {
            ...
            const tkCell = !r.algunConDatos
                ? '<span title="Sin información de tickets disponible" style="color:#C9BCBD">—</span>'
                : `<b>${fN(r.tickets)}</b>${r.sinTickets ? ' <span title="No incluye tickets de sucursal(es) sin información disponible" style="color:#C9BCBD;font-weight:700">*</span>' : ''}`;
        ...
        tbody.innerHTML = rows + `<tr class="total-row"><td data-label="" colspan="2"><b>TOTAL PERÍODO</b></td><td class="r" data-label="Unidades">${fN(tUn)}</td><td class="r" data-label="Ventas $">${fF(tV)}</td><td class="r" data-label="Utilidad">${fF(tU)}</td><td class="r" data-label="Margen"><span class="pill ${totMg>=.50?'hi':'mi'}">${fP(totMg)}</span></td><td class="r" data-label="Tickets">${totTkTexto}</td></tr>`
            + (huboSinTickets ? `<tr><td colspan="7" style="font-size:.65rem;color:#AD9C9F;text-align:right;border:none;padding-top:4px">${notaPie}</td></tr>` : '');
    }
    function updateChartGeneric(chartId, field, sourceData, dates, byDate, activeSucs, multi){
        ...
        const monoColor = isVentas ? '#E31B23' : '#2196A8';
            ...
            options:{
                ...
                plugins:{
                    legend:{display:multi, position:'top', onClick:null,
                        labels:{boxWidth:8,boxHeight:8,usePointStyle:true,pointStyle:'circle',padding:12,color:'#665E60',font:{family:"'Segoe UI', sans-serif",size:10,weight:'600'}}},
                    datalabels:{display:false},
                    tooltip:PREMIUM_TOOLTIP_OPTS
                },
                scales:{
                    x:{grid:{display:false}, ticks:{font:{size:10},color:'#8C8082'}},
                    y:{grid:{color:'#F8F4F4'}, ticks:{font:{size:10},color:'#8C8082', callback: isVentas ? v=>fM(v) : v=>v}}
                }
            }
        });
    }
        ...
        charts['lineas'] = new Chart(document.getElementById('chart-lineas'), {
            ...
            options:{
                ...
                plugins:{
                    ...
                    datalabels:{display:true,anchor:'end',align:'end',color:'#544C4E',font:{weight:'600',size:9.5},
                        formatter:(value,ctx)=>{const t=ctx.chart.data.datasets[ctx.datasetIndex].data.reduce((a,b)=>a+b,0); return fP(t>0?value/t:0);}}
                },
                scales:{
                    x:{grid:{display:false},border:{display:false},suggestedMax:maxVal*1.15,ticks:{font:{size:9},color:'#8C8082',callback:currentLineMetric==='unidades'?v=>fN(v):v=>fM(v)}},
                    y:{grid:{display:false},border:{display:false},ticks:{font:{size:10,weight:'600'},color:'#262123'}}
                }
            }
        });
        fitTables();
    ...
    function updateLineasActual(){
        ...
        charts['lineasactual'] = new Chart(document.getElementById('chart-lineasactual'), {
            ...
            options:{
                ...
                scales:{
                    x:{grid:{display:false},border:{display:false},suggestedMax:maxVal*1.15,ticks:{font:{size:9},color:'#8C8082',callback:currentLineMetricActual==='unidades'?v=>fN(v):v=>fM(v)}},
                    y:{grid:{display:false},border:{display:false},ticks:{font:{size:10,weight:'600'},color:'#262123'}}
                }
            }
        });
    ...
    function updateTablaSucursalActual(){
        ...
        const rows = data.map(r => {
            ...
            const tkCell = !r.algunConDatos
                ? '<span title="Sin información de tickets disponible" style="color:#C9BCBD">—</span>'
                : `${fN(r.tickets)}${r.sinTickets ? ' <span title="Parcial: algunas fechas sin información de tickets" style="color:#C9BCBD;font-weight:700">*</span>' : ''}`;
        ...
        tbody.innerHTML = rows + `<tr class="total-row"><td data-label=""><b>TOTAL MES EN CURSO</b></td><td class="r" data-label="Unidades">${fN(tUn)}</td><td class="r" data-label="Ventas $">${fF(tV)}</td><td class="r" data-label="Utilidad">${fF(tU)}</td><td class="r" data-label="Margen"><span class="pill ${totMg>=.50?'hi':'mi'}">${fP(totMg)}</span></td><td class="r" data-label="Tickets">${totTkTexto}</td></tr>`
            + (huboSinTickets ? `<tr><td colspan="6" style="font-size:.65rem;color:#AD9C9F;text-align:right;border:none;padding-top:4px">${notaPie}</td></tr>` : '');
    }
            ...
            options:{
                ...
                plugins:{
                    legend:{display:true, position:'top', onClick:null,
                        labels:{boxWidth:8,boxHeight:8,usePointStyle:true,pointStyle:'circle',padding:10,color:'#665E60',font:{family:"'Segoe UI', sans-serif",size:10,weight:'600'}}},
                    datalabels:{display:false},
                ...
                scales:{
                    x:{grid:{display:false}, ticks:{font:{size:9},color:'#8C8082',maxRotation:45}},
                    y:{position:'left', grid:{color:'#F8F4F4'}, ticks:{font:{size:9},color:'#8C8082', callback: v => isMoneda?fM(v):fN(v)}},
    ...
    function updateHistorico(){
        ...
        tbody.innerHTML = rows + `<tr class="total-row"><td data-label=""><b>TOTAL HISTÓRICO</b></td><td class="r" data-label="Unidades">${fN(tUn)}</td><td class="r" data-label="Ventas $">${fF(tV)}</td><td class="r" data-label="Utilidad">${fF(tU)}</td><td class="r" data-label="Margen"><span class="pill ${totMg>=.50?'hi':'mi'}">${fP(totMg)}</span></td><td class="r" data-label="Tickets">${totTkTexto}</td></tr>`
            + (huboSinTickets ? `<tr><td colspan="6" style="font-size:.65rem;color:#AD9C9F;text-align:right;border:none;padding-top:4px">${notaPieHist}</td></tr>` : '');
        const activeSucs = getActiveSucs();
        const multi      = activeSucs.length > 1;
        ...
        const barDatasets = multi
            ...
            : [{
                ...
                backgroundColor:'#E31B23cc', borderColor:'#E31B23',
        ...
        charts['historico'] = new Chart(document.getElementById('chart-historico'), {
            ...
            options:{
                ...
                plugins:{
                    legend:{display:true, position:'top', onClick:null,
                        labels:{boxWidth:8,boxHeight:8,usePointStyle:true,pointStyle:'circle',padding:10,color:'#665E60',
                            font:{family:"'Segoe UI', sans-serif",size:10,weight:'600'}}},
                    datalabels:{display:false},
                    tooltip:{...PREMIUM_TOOLTIP_OPTS, callbacks:{
                        label: ctx => {
                ...
                scales:{
                    x:{grid:{display:false}, ticks:{font:{size:9},color:'#8C8082',maxRotation:45}},
                    y:{stacked:true, position:'left', grid:{color:'#F8F4F4'}, ticks:{font:{size:9},color:'#8C8082', callback: v => isMoneda?fM(v):fN(v)}},
                    y2:{position:'right', grid:{display:false}, ticks:{font:{size:9},color:'#aa7300', callback: v => v.toFixed(1)+'%'}}
                }
            }
        });
    ...
    function renderComparativoVentas(canvasId, chartKey, labelRef, valRef, labelAct, valAct){
        ...
        charts[chartKey] = new Chart(document.getElementById(canvasId), {
            ...
            data: {
                ...
                datasets: [{
                    data: [valRef, valAct],
                    backgroundColor: ['#DBBCC2', '#E31B23'],
                    borderColor: ['#C9888F', '#852427'],
            ...
            options: {
                ...
                plugins: {
                    ...
                    datalabels: {
                        ...
                        color: '#544C4E', font: { weight: '700', size: 11 },
                        formatter: v => fF(v)
                    },
                    tooltip: { ...PREMIUM_TOOLTIP_OPTS, callbacks: {
                ...
                scales: {
                    x: { grid: { display: false }, ticks: { font: { size: 10, weight: '600' }, color: '#665E60' } },
                    y: { grid: { color: '#F8F4F4' }, suggestedMax: max > 0 ? max * 1.18 : 1, ticks: { font: { size: 9 }, color: '#8C8082', callback: v => fM(v) } }
                }
            }
        });
    }
    ...
    function updateTopArticulos(){
        ...
        charts['bubble'] = new Chart(document.getElementById('chart-bubble'), {
            ...
            options: {
                ...
                scales: {
                    x: {
                        title: { display: true, text: 'Ventas $ c/Desc', color: '#8C8082', font: { size: 10, weight: '600' } },
                        grid: { color: '#F8F4F4' },
                        ticks: { font: { size: 9 }, color: '#8C8082', callback: v => fM(v) }
                    },
                    y: {
                        title: { display: true, text: 'Margen %', color: '#8C8082', font: { size: 10, weight: '600' } },
                        grid: { color: '#F8F4F4' },
                        ticks: { font: { size: 9 }, color: '#8C8082', callback: v => v.toFixed(1)+'%' }
                    }
                }
            }
        });
    ...
    const QUART_COLORS = {
        1: {bg:'#E31B23', fg:'#ffffff', tint:'#E31B2314'},
        2: {bg:'#BE6C6E', fg:'#ffffff', tint:'#BE6C6E12'},
        3: {bg:'#ECC6CA', fg:'#5C3A3F', tint:'#ECC6CA20'},
        4: {bg:'#F6EAEB', fg:'#756567', tint:'#F6EAEB40'},
    };
        ...
        [1,2,3,4].forEach(q => {
            ...
            html += `<tr class="fq-parent" onclick="toggleFabQuartil('${rid}')" style="background:${qc.tint}">
                ...
                <td data-label="Cuartil"><span class="fq-badge" style="background:${qc.bg};color:${qc.fg}">${QUART_LABELS[q]}</span>
                    <span style="color:#8C8082;font-size:.68rem;margin-left:6px;display:block;text-align:left;margin-top:4px">${items.length} fabricante${items.length!==1?'s':''} · ${fP(startPct)}–${fP(endPct)} acumulado</span></td>
            ...
            options: {
                ...
                plugins: {
                    ...
                    datalabels: {
                        ...
                        color: '#544C4E', font: { weight: '600', size: 9.5 },
                        formatter: (value, ctx) => fP(paretoList[ctx.dataIndex].pct)
                    }
                },
                scales: {
                    x: { grid:{display:false}, border:{display:false}, suggestedMax:maxVal*1.15, ticks:{font:{size:9},color:'#8C8082',callback:v=>fM(v)} },
                    y: { grid:{display:false}, border:{display:false}, ticks:{font:{size:10,weight:'600'},color:'#262123'} }
                }
            }
        });
